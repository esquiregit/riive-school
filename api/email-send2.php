	            		...
	            		<div class="col-md-12">
	            			<div class="form-group">
                                ...
                                <select id="phone_number" name="phone_number" class="form-control show">
                                    <option value="default" <?php if(isset($contact_number) && $contact_number == 'default') echo 'selected'; ?>>-- Select Receiver --</option>
                                    <?php foreach($parents_emails as $contact){
                                        $name = empty($contact->othernames) ? $contact->firstname . ' ' . $contact->lastname : $contact->firstname . ' ' . $contact->othernames . ' ' . $contact->lastname;
                                    ?>
                                    <option title="<?php echo $contact->email ?>" value="<?php echo $contact->email ?>" <?php if(isset($contact_number) && $contact_number == $contact->email) echo 'selected'; ?>><?php echo Methods::strtocapital($name) . "'s " . ucfirst($contact->relation); ?></option>
                                    <?php } ?>
                                </select>

                                <select multiple="multiple" style="height: auto !important" id="phone_numbers" name="phone_numbers[]" class="form-control hide">
                                    <option value="default" <?php if(isset($contact_number) && $contact_number == 'default') echo 'selected'; ?>>-- Select Receivers --</option>
                                    <?php foreach($parents_emails as $contact){
                                        $name = empty($contact->othernames) ? $contact->firstname . ' ' . $contact->lastname : $contact->firstname . ' ' . $contact->othernames . ' ' . $contact->lastname;
                                    ?>
                                    <option title="<?php echo $contact->email ?>" value="<?php echo $contact->email ?>" <?php if(isset($contact_number) && $contact_number == $contact->email) echo 'selected'; ?>><?php echo Methods::strtocapital($name) . "'s " . ucfirst($contact->relation); ?></option>
