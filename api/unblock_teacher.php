<?php
	...
	if(!isset($request->teacher_id) || !isset($request->schoolCode)) {
		...
		if(Teacher::change_teacher_status($teacher_id, $schoolCode, 'Active', $conn)) {
			array_push($response, array(
				"status"  => "Success",
				"message" => "Teacher Unblocked Successfully....."
			));
	        Audit_Trail::create_log($schoolCode, '', 'Unblocked Teacher "'.$name.'"', $conn);
		} else {
			array_push($response, array(
				"status"  => "Error",
				"message" => "Teacher Could Not Be Unblocked. Please Try Again....."
			));
	        Audit_Trail::create_log($schoolCode, '', 'Tried To Unblock Teacher "'.$name.'"', $conn);
		}
