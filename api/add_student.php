<?php
	...
	if(!isset($firstname) || !isset($lastname) || !isset($school_id) || !isset($name) || !isset($gender) || !isset($dob) || !isset($access_level) || !isset($othernames)) {
		...
		if(empty($firstname) && empty($lastname) && empty($gender) && empty($class) && empty($dob) && empty($profile_picture)) {
        	...
        	$username    = strtolower($access_level) === 'school' ? Student::read_school_username_by_id($school_id, $conn) : Teacher::read_teacher_username_by_id($school_id, $conn);
			if(empty($profile_picture)) {
				if(Student::create_student($school_id, $school_name, $firstname, $lastname, $othernames, $gender, $dob, $class, 'avatar.png', $conn)) {
					array_push($response, array(
						"status"  => "Success",
						"message" => "Student Details Added Successfully...."
					));
	        		Audit_Trail::create_log($school_id, $teacher_id, 'Added Student "'.$firstname.' '.$lastname.'" To Class "'.$class.'"', $conn);
				} else {
					array_push($response, array(
						"status"  => "Error",
						"message" => "Student Details Could Not Be Added. Please Try Again...."
					));
	        		Audit_Trail::create_log($school_id, $teacher_id, 'Tried To Add Student "'.$firstname.' '.$lastname.'" To Class "'.$class.'"', $conn);
				}
			} else  {
				...
				if($_FILES['image'] && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
					if(Student::create_student($school_id, $school_name, $firstname, $lastname, $othernames, $gender, $dob, $class, $picture_upload, $conn)) {
