...
function Email({ history }) {
    ...
    useEffect(() => {
        document.title = 'RiiVe | Email';
        const abortController = new AbortController();
        const signal = abortController.signal;

        if (user) {
            if (user.access_level.toLowerCase() === 'school') {
                Axios.post(getBaseURL() + 'get_parents_message_email_addresses', { school_id: user.id }, { signal: signal })
                    .then(response => {
                ...
                Axios.post(getBaseURL() + 'get_parents_message_email_addresses', { school_id: user.id, class: user.class }, { signal: signal })
                    .then(response => {
                        setParents(response.data);
                        setLoading(false);
                    })
    ...
    const handleChange   = (event, newValue) => {
        if(event) {
            ...
            if(newValue) {
                ...
                newArr[1]['subject'] = value;
            } else if(name === 'attachment') {
                newArr[2]['attachment'] = file;
            } else if(name === 'message') {
                newArr[3]['message'] = value;
            }
            setValues(newArr);
    ...
    const flipSwitch     = () => {
        ...
        const message    = values[3].message;
    ...
    const onSubmit       = (action) => {
        ...
        if(action.toLowerCase() === 'yes') {
            ...
            formData.append('message',      values[3].message);
            formData.append('subject',      values[1].subject);
            formData.append('recipient',    recipients);
            formData.append('school_id',    user && user.access_level.toLowerCase() === 'school' ? user.id : user.school_id);
            formData.append('attachment',   values[2].attachment);
            formData.append('teacher_id',   user && user.access_level.toLowerCase() === 'teacher' ? user.id : '');
            formData.append('access_level', user && user.access_level);
            
            setError(false);
            setBackdrop(true);
            setComError(false);
            Axios.post(getBaseURL()+'send_email', formData)
                .then(response => {
    ...
    return (
        <>
            ...
            <Backdrop className={classes.backdrop} open={backdrop}>
                <CircularProgress color="inherit" /> <span className='ml-15'>Sending Email....</span>
            </Backdrop>
            { isConfirmOpen &&
                <div>
                    <Dialog
